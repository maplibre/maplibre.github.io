...
Happy New Year, MapLibre Community! This month, we’re thrilled to share big updates for MapLibre Native (including Vulkan support), the official launch of MapLibre GL JS Version 5.0, new features in Martin Tile Server, and more.
We’ve also added a new meeting hour to better connect with contributors around the world.
...
The library integrates MapLibre iOS and MapLibre Android and is open to contributions. The README lists various ideas, including adding support for web (presumably by intergrating MapLibre GL JS), which would make the library truly cross-platform.
...
- **SDF sprites**: `SDF` sprites can now be generated via a separate API route. This allows to color sprites in `maplibre-gl`/`maplibre-native` based on map data and have crispier sprites.
  ...
  - Background customization limited to blurred borders.
    ...
    Learn more in [our docs](https://maplibre.org/martin/sources-sprites.html). Here’s an example sprite from [NavigaTUM](https://nav.tum.de):
...
- January 29, 2025 - 9:00–10:00 AM UTC
